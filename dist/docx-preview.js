...
(function (global, factory) {
...
})(this, (function (exports, JSZip$1) { 'use strict';
    ...
    class OpenXmlPackage {
        ...
        static async load(input, options) {
            const zip = await JSZip$1.loadAsync(input);
...
`;
    ...
    function processInlineNodes(node) {
        ...
        for (const child of Array.from(node.childNodes)) {
            if (child.nodeType === 3) {
                ...
                const el = child;
                const tag = el.tagName.toUpperCase();
                if (['STYLE', 'SCRIPT', 'LINK', 'META', 'NOSCRIPT'].includes(tag))
                    continue;
                if (tag === 'BR') {
                    runs += `<w:r><w:br/></w:r>`;
                }
        ...
        if (tag === 'UL' || tag === 'OL') {
            ...
            for (const li of Array.from(el.children)) {
                if (li.tagName?.toUpperCase() === 'LI') {
                    xml += processListItem(li, { numId, level: (listInfo?.level ?? -1) + 1});
                }
            }
            return xml;
        }
    ...
    function processListItem(li, listInfo) {
        let xml = '';
        const hasBlockChildren = Array.from(li.children).some(c => isBlockElement(c) && !['UL', 'OL'].includes(c.tagName.toUpperCase()));
        if (hasBlockChildren) {
            let isFirst = true;
            for (const child of Array.from(li.childNodes)) {
                if (child.nodeType === 1 && isBlockElement(child)) {
                    if (['UL', 'OL'].includes(child.tagName.toUpperCase())) {
                        ...
                        xml += processBlockElement(child, isFirst ? listInfo : undefined);
                        isFirst = false;
                    }
                }
    ...
    function buildDocumentXml(editorEl) {
        ...
        for (const child of Array.from(children)) {
            if (child.nodeType === 1) {
                ...
                if (skipTags.includes(el.tagName?.toUpperCase()))
                    continue;
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    ...
    async function exportAndDownload(editorElement, filename) {
        filename = filename || 'document.docx';
        const finalName = filename.endsWith('.docx') ? filename : filename + '.docx';
        const blob = await exportToDocx(editorElement);
    ...
    class DocxEditor {
        constructor(container, options) {
            ...
            if (!container)
                throw new Error('DocxEditor: container element is required');
